...
export interface IStorage {
  ...
  // Admin methods
  updateUserActivity(userId: string, activity: { lastLoginAt?: Date; lastLogoutAt?: Date; isOnline?: boolean; deviceType?: string }): Promise<void>;
  getAllUsers(): Promise<User[]>;
  getAllUsersWithStats(): Promise<Array<User & { stats?: UserStats }>>;
  getUserSessions(userId: string): Promise<any[]>;
  createUserSession(userId: string, session: { deviceType?: string; userAgent?: string; ipAddress?: string }): Promise<any>;
...
export class MemStorage implements IStorage {
  ...
  // Admin methods - in-memory implementations
  async updateUserActivity(userId: string, activity: { lastLoginAt?: Date; lastLogoutAt?: Date; isOnline?: boolean; deviceType?: string }): Promise<void> {
    const user = this.users.get(userId);
    if (user) {
      this.users.set(userId, { ...user, ...activity });
    }
  }
  ...
  async getAllUsersWithStats(): Promise<Array<User & { stats?: UserStats }>> {
    const allUsers = Array.from(this.users.values());
    return allUsers.map(user => ({
      ...user,
      stats: Array.from(this.userStats.values()).find(stat => stat.userId === user.id)
    })).sort((a, b) => 
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
  }
  ...
  async getUserSessions(userId: string): Promise<any[]> {
    // In-memory doesn't track sessions, return empty array
    return [];
  }

  async createUserSession(userId: string, session: { deviceType?: string; userAgent?: string; ipAddress?: string }): Promise<any> {
    // In-memory doesn't track sessions
    return { id: randomUUID(), userId, ...session, sessionStart: new Date() };
  }
