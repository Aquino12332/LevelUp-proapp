...
import {
  ...
  focusSessions, friendRequests, friendships, userSessions,
...
export class DbStorage implements IStorage {
  ...
  async getAllUsersWithStats(): Promise<Array<User & { stats?: UserStats }>> {
    ...
    return allUsers.map(user => ({
      ...user,
      stats: allStats.find(stat => stat.userId === user.id)
    }));
  }
  ...
  async endUserSession(sessionId: string): Promise<void> {
    await db.update(userSessions).set({ sessionEnd: new Date() }).where(eq(userSessions.id, sessionId));
  }
}
