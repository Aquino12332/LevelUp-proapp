...
export const users = pgTable("users", {
  ...
  // notificationPreferences: text("notificationPreferences"), // JSON: {dueReminderMinutes: 60, overdueEnabled: true, recurringEnabled: true} - COMMENTED OUT until migration
});
...
export const tasks = pgTable("tasks", {
  ...
  // Notification tracking fields - COMMENTED OUT until database migration is run
...
// User activity sessions table for tracking app open/close
export const userSessions = pgTable("userSessions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("userId").notNull(),
  sessionStart: timestamp("sessionStart").notNull().defaultNow(),
...
export const dailyMetrics = pgTable("dailyMetrics", {
  ...
  totalStudyTime: varchar("totalStudyTime").default("0"), // minutes (changed to varchar for consistency)
  focusSessions: varchar("focusSessions").default("0"),
  tasksCompleted: varchar("tasksCompleted").default("0"),
  tasksCreated: varchar("tasksCreated").default("0"),
  notesCreated: varchar("notesCreated").default("0"),
  loginCount: varchar("loginCount").default("0"),
  sessionDuration: varchar("sessionDuration").default("0"), // total app time in minutes
  createdAt: timestamp("createdAt").notNull().defaultNow(),
}, (table) => ({
